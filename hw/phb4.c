...
static inline uint64_t phb4_set_sm_timeout(struct phb4 *p, uint64_t dur)
{
...
static void phb4_device_init(struct phb *phb, struct pci_device *dev)
{
	int ecap = 0;
	int aercap = 0;
...
static int64_t phb4_pci_reinit(struct phb *phb, uint64_t scope, uint64_t data)
{
	...
	phb4_device_init(phb, pd);
	return OPAL_SUCCESS;
}

static int64_t phb4_presence_detect(struct phb *phb)
{
	struct phb4 *p = phb_to_phb4(phb);
	uint64_t hps, dtctl;
...
static void phb4_init_ioda_cache(struct phb4 *p)
{
	...
	memset(p->mdt_cache, 0x0, sizeof(p->mdt_cache));
	memset(p->mbt_cache, 0x0, sizeof(p->mbt_cache));
	...
	p->mbt_cache[0][1] = IODA3_MBT0_ENABLE |
		((~(M32_PCI_SIZE - 1)) & IODA3_MBT1_MASK_64);
}
...
static int64_t phb4_set_phb_mem_window(struct phb __unused *phb,
				       uint16_t __unused window_type,
				       uint16_t __unused window_num,
				       uint64_t __unused addr,
				       uint64_t __unused pci_addr,
				       uint64_t __unused size)
{
	return OPAL_UNSUPPORTED;
}
				       ...
				       uint16_t segment_num)
{
	...
	/*
	 * PHB4 doesn't support IODT any more. On the other
	 * hand, PHB4 support M64DT with much more flexibility.
	...
	case OPAL_M32_WINDOW_TYPE:
		...
		cache = &p->mdt_cache[segment_num];
		phb4_ioda_sel(p, IODA3_TBL_MDT, segment_num, false);
		out_be64(p->regs + PHB_IODA_DATA0,
			 SETFIELD(IODA3_MDT_PE_A, 0ull, pe_num));
		*cache = SETFIELD(IODA3_MDT_PE_A, 0ull, pe_num);
...
static int64_t phb4_link_state(struct phb *phb)
{
	struct phb4 *p = phb_to_phb4(phb);
	...
	rc = phb4_pcicfg_read16(&p->phb, 0, p->ecap + PCICAP_EXP_LSTAT,
				&lstat);
	if (rc < 0) {
		/* Shouldn't happen */
...
static int64_t phb4_power_state(struct phb __unused *phb)
{
	/* XXX Test for PHB in error state ? */
...
static int64_t phb4_slot_power_off(struct phb *phb)
{
	struct phb4 *p = phb_to_phb4(phb);

	if (p->state == PHB4_STATE_BROKEN)
		return OPAL_HARDWARE;
	if (p->state != PHB4_STATE_FUNCTIONAL)
		return OPAL_BUSY;
...
static int64_t phb4_slot_power_on(struct phb *phb)
{
	struct phb4 *p = phb_to_phb4(phb);
...
static void phb4_setup_for_link_down(struct phb4 *p)
{
	uint32_t reg32;
	...
	phb4_pcicfg_read32(&p->phb, 0, p->aercap + PCIECAP_AER_CE_MASK, &reg32);
	reg32 |= PCIECAP_AER_CE_RECVR_ERR;
	phb4_pcicfg_write32(&p->phb, 0, p->aercap + PCIECAP_AER_CE_MASK, reg32);
}

static void phb4_setup_for_link_up(struct phb4 *p)
{
	uint32_t reg32;
	 ...
	 */
	if (p->flags & PHB4_RESTORE_BUS_NUM) {
		p->flags &= ~PHB4_RESTORE_BUS_NUM;
		pci_restore_bridge_buses(&p->phb);
	}
}

static int64_t phb4_sm_link_poll(struct phb4 *p)
{
	uint64_t reg;
	 ...
	 */
	switch(p->state) {
	case PHB4_STATE_WAIT_LINK_ELECTRICAL:
		...
		if (reg & (PHB_PCIE_DLP_INBAND_PRESENCE |
			...
			p->state = PHB4_STATE_WAIT_LINK;
			p->retries = PHB4_LINK_WAIT_RETRIES;
		} else if (p->retries-- == 0) {
		...
		return phb4_set_sm_timeout(p, msecs_to_tb(100));
	case PHB4_STATE_WAIT_LINK:
		reg = in_be64(p->regs + PHB_PCIE_DLP_TRAIN_CTL);
		if (reg & PHB_PCIE_DLP_TL_LINKACT) {
			/* Setup PHB for link up */
			phb4_setup_for_link_up(p);
			PHBDBG(p, "Link is up!\n");
			p->state = PHB4_STATE_FUNCTIONAL;
			return OPAL_SUCCESS;
		}
...
static int64_t phb4_start_link_poll(struct phb4 *p)
{
	/*
	 ...
	 */
	p->retries = PHB4_LINK_ELECTRICAL_RETRIES;
	p->state = PHB4_STATE_WAIT_LINK_ELECTRICAL;
	return phb4_set_sm_timeout(p, msecs_to_tb(100));
}

static int64_t phb4_sm_hot_reset(struct phb4 *p)
{
	uint16_t brctl;

	switch (p->state) {
	case PHB4_STATE_FUNCTIONAL:
		/* We need do nothing with available slot */
		if (phb4_presence_detect(&p->phb) != OPAL_SHPC_DEV_PRESENT) {
			PHBDBG(p, "Slot hreset: no device\n");
			return OPAL_CLOSED;
		}

		/* Prepare for link going down */
		phb4_setup_for_link_down(p);
		...
		p->state = PHB4_STATE_HRESET_DELAY;
		return phb4_set_sm_timeout(p, secs_to_tb(1));
	case PHB4_STATE_HRESET_DELAY:
		/* Turn off hot reset */
		phb4_pcicfg_read16(&p->phb, 0, PCI_CFG_BRCTL, &brctl);
		brctl &= ~PCI_CFG_BRCTL_SECONDARY_RESET;
		phb4_pcicfg_write16(&p->phb, 0, PCI_CFG_BRCTL, brctl);
		 ...
		 */
		p->state = PHB4_STATE_HRESET_DELAY2;
		return phb4_set_sm_timeout(p, secs_to_tb(1));
	case PHB4_STATE_HRESET_DELAY2:
		return phb4_start_link_poll(p);
	default:
...
static int64_t phb4_hot_reset(struct phb *phb)
{
	struct phb4 *p = phb_to_phb4(phb);

	if (p->state != PHB4_STATE_FUNCTIONAL) {
		PHBDBG(p, "phb4_hot_reset: wrong state %d\n",
		       p->state);
		return OPAL_HARDWARE;
	}

	p->flags |= PHB4_CFG_BLOCKED;
	return phb4_sm_hot_reset(p);
}

static int64_t phb4_sm_fundamental_reset(struct phb4 *p)
{
	uint64_t reg;
	 ...
	 */
	if (p->state == PHB4_STATE_FUNCTIONAL &&
	    phb4_presence_detect(&p->phb) != OPAL_SHPC_DEV_PRESENT) {
		PHBDBG(p, "Slot freset: no device\n");
		return OPAL_CLOSED;
	}

	/* Handle boot time skipping of reset */
	if (p->skip_perst && p->state == PHB4_STATE_FUNCTIONAL) {
		PHBINF(p, "Cold boot, skipping PERST assertion\n");
		p->state = PHB4_STATE_FRESET_ASSERT_DELAY;
	...
	case PHB4_STATE_FRESET_ASSERT_DELAY:
		/* Deassert PERST */
		reg = in_be64(p->regs + PHB_PCIE_CRESET);
		reg |= PHB_PCIE_CRESET_PERST_N;
		out_be64(p->regs + PHB_PCIE_CRESET, reg);
		...
		p->state = PHB4_STATE_FRESET_DEASSERT_DELAY;

		/* CAPP fpga requires 1s to flash before polling link */
		return phb4_set_sm_timeout(p, secs_to_tb(1));

	case PHB4_STATE_FRESET_DEASSERT_DELAY:
...
static int64_t phb4_fundamental_reset(struct phb *phb)
{
	struct phb4 *p = phb_to_phb4(phb);

	if (p->state != PHB4_STATE_FUNCTIONAL) {
		PHBDBG(p, "phb4_fundamental_reset: wrong state %d\n", p->state);
		return OPAL_HARDWARE;
	}

	p->flags |= PHB4_CFG_BLOCKED;
	return phb4_sm_fundamental_reset(p);
}
 ...
 */
static int64_t phb4_sm_complete_reset(struct phb4 *p)
{
	// XXX reset PHB

	switch (p->state) {
	case PHB4_STATE_FENCED:
	case PHB4_STATE_FUNCTIONAL:

		p->state = PHB4_STATE_CRESET_WAIT_CQ;
		p->retries = 500;
		return phb4_set_sm_timeout(p, msecs_to_tb(10));
	case PHB4_STATE_CRESET_WAIT_CQ:
		p->state = PHB4_STATE_CRESET_REINIT;
		return phb4_set_sm_timeout(p, msecs_to_tb(100));

	case PHB4_STATE_CRESET_REINIT:
		p->flags &= ~PHB4_AIB_FENCED;
		p->flags &= ~PHB4_CAPP_RECOVERY;
		phb4_init_hw(p, false);

		p->state = PHB4_STATE_CRESET_FRESET;
		return phb4_set_sm_timeout(p, msecs_to_tb(100));
	case PHB4_STATE_CRESET_FRESET:
		p->state = PHB4_STATE_FUNCTIONAL;
		p->flags |= PHB4_CFG_BLOCKED;
		return phb4_sm_fundamental_reset(p);
	default:
...
static int64_t phb4_complete_reset(struct phb *phb, uint8_t assert)
{
	struct phb4 *p = phb_to_phb4(phb);
	...
	if ((assert == OPAL_ASSERT_RESET &&
	    ...
	    p->state != PHB4_STATE_FUNCTIONAL)) {
		PHBERR(p, "phb4_creset: wrong state %d\n",
		       p->state);
		return OPAL_HARDWARE;
	}
	...
	if (assert == OPAL_ASSERT_RESET) {
		...
		return phb4_sm_complete_reset(p);
	} else {
...
static int64_t phb4_poll(struct phb *phb)
{
	struct phb4 *p = phb_to_phb4(phb);
	uint64_t now = mftb();
	...
	case PHB4_STATE_WAIT_LINK_ELECTRICAL:
	case PHB4_STATE_WAIT_LINK:
		return phb4_sm_link_poll(p);
	default:
		PHBDBG(p, "phb4_poll: wrong state %d\n", p->state);
		break;
	}
...
static const struct phb_ops phb4_ops = {
	...
	.presence_detect	= phb4_presence_detect,
	...
	.link_state		= phb4_link_state,
	.power_state		= phb4_power_state,
	.slot_power_off		= phb4_slot_power_off,
	.slot_power_on		= phb4_slot_power_on,
	.hot_reset		= phb4_hot_reset,
	.fundamental_reset	= phb4_fundamental_reset,
	.complete_reset		= phb4_complete_reset,
	.poll			= phb4_poll,
...
static void phb4_init_ioda3(struct phb4 *p)
{
	...
	out_be64(p->regs + PHB_LSI_SOURCE_ID, SETFIELD(PHB_LSI_SRC_ID, 0ull, 0x1ff));
			     ...
			     uint32_t nest_base, uint32_t pci_base)
{
	...
	if (mmio0_bar == 0 || force_assign) {
		...
		mmio0_bmask =  (~MMIO0_BAR_SIZE) & 0x00FFFFFFFFFFFFFFULL;
		...
		mmio1_bmask =  (~MMIO1_BAR_SIZE) & 0x00FFFFFFFFFFFFFFULL;
