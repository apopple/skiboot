				  ...
				  uint32_t *data, bool write)
{
	...
	for (i = 0; i < ARRAY_SIZE(bar_map); i++) {
		...
		if (enabled)
			...
			bar->flags &= ~NPU2_BAR_FLAG_ENABLED;
		npu2_write_bar(ndev->npu, bar, 0, 0);
	}

	return OPAL_PARTIAL;
}

static int64_t npu2_cfg_read_bar(struct pci_cfg_reg_filter *pcrf,
				 uint32_t offset, uint32_t size,
				 uint32_t *data)
{
				...
				bool write)
{
	...
	return npu2_cfg_read_bar(pcrf, offset, len, data);
}
...
static void npu2_create_memory_node(uint64_t addr, uint64_t size)
{
	...
	dt_add_property_cells(mem, "linux,usable-memory", 0, 0, 0, 0);
...
static void npu2_hw_init(struct npu2 *p)
{
	...
	npu2_create_memory_node(0x0002030000000000, 16*1024*1024*1024UL);
				  ...
				  struct dt_node *dn)
{
	...
	dt_for_each_compatible(npu2_dn, link, "ibm,npu-link") {
		...
		/* Populate BARs */
		dev->bars[0]   = &npu2_bars[3 + dev->index];
		dev->bars[1] = &npu2_bars[9 + dev->index / 2];
